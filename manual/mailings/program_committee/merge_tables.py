...
reviews = np.loadtxt(sys.argv[1], delimiter=",", dtype=review_dtype)
...
ids = dict.fromkeys(
    [s.lower().strip() for s in reviews["id"]], np.array([], dtype=review_dtype)
)
...
for _id, id_reviews in ids.iteritems():
    ...
    for category in norm_cats:
        for review in id_reviews:
            ...
            if reviewer_std != 0.0:
                summary_scores[ii]["overall"] += (
                    norm_cats_weights[category]
                    * (review[category] - reviewer_mean)
                    / reviewer_std
                )
...
abstract_dtype = np.dtype(
    {
        "names": ("id", "speaker", "title", "category", "domain", "type"),
